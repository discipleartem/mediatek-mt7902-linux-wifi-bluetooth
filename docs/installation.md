...
- Kernel **6.6–7.0** for current backports (mainline MT7902 expected in **7.1+**)
...
Aliases: `all` → `install-all`, `bt` → `bluetooth`, `restore` → `rollback`.

Installer layout: thin `mt7902.sh` + `lib/*.sh` (Wi‑Fi, Bluetooth, systemd, backup, verify).
